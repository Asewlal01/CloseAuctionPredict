...
from Modeling.DatasetManagers.BaseDatasetManager import BaseDatasetManager, convert_to_classification
...
def train(model: BaseModel, train_data: tuple[torch.Tensor, torch.Tensor], epochs: int, learning_rate: float,
    ...
    loader = create_loader(train_data, batch_size)
    ...
    for epoch in range(epochs):
        ...
        if verbose:
            print(f"Epoch {epoch+1}/{epochs} has average Loss of : {total_loss:.4f}")

def evaluate(model, dataset, batch_size=1024) -> tuple[float, float, float]:
    """
...
def create_loader(dataset: tuple[torch.Tensor, torch.Tensor], batch_size: int = 32) -> DataLoader:
    """
...
def compute_sample_weights(y):
    ...
    total_return = abs_return.sum(dim=0, keepdim=True)
